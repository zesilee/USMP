...
**namespace 坑**：BGP 用 `urn:huawei:yang:huawei-bgp`（8.20.10 YANG 声明的权威 module namespace，用户拍板）。⚠️ 当时预警的「VLAN/IFM 旧式 `urn:huawei:params:xml:ns:yang:huawei-<mod>` 潜在真机隐患」**已在真机应验**（2026-08-04 真机创建 VLAN 报 `rpc-error unknown-namespace: vlan has invalid namespace`）并修复：全仓统一为设备声明的 `urn:huawei:yang:huawei-<mod>`，新增守护测试 `internal/drivers/huawei_namespace_test.go`（以 `snd/ce6866p-yang/` 为真理源，逐描述符+per-node map 校验，新注册模块自动纳入）——「sim namespace-宽容、只有真机才校验」的盲区已被该守护补上。谓词用 `HasPrefix("/bgp:bgp")`（比 Contains 精确，排除 bgp-flow:/bgp-evpn:/per-VPN）。
