...
- [iView组件库调研](view-ui-insights.md) — 前端选型/借鉴前必读：不换库(430star vs 27.7k+单人维护)、iview包谱系防坑(旧包已死)、可借鉴=台账CSV导出/LoadingBar/左树可拖拽、14px+32px+蓝主色=行业事实标准再印证；全文 docs/research/view-ui-insights.md
...
- [删除语义](config-delete-semantics.md) — 改写链路/删除/对账/netconfsim前必读：**不删除契约两道闸(walkMap subset + Reconcile 的 desired==nil 空返回,BIO-05)，diff.walk 的 desiredNil→DeleteChange 分支是陷阱勿据此"修"对账否则删真机配置(PR#418 已加锁定用例)**；声明式通道刻意删不了(walkMap subset)→DELETE命令通道、先移desired再下发、sim已接RFC edit-config(整树替换退役)、opMu写事务串行化、华为vlan key是id；**PR#145「内置接口删不掉」根因=写链路漏发模块顶层容器`<ifm>/<vlan>`发扁平根→设备嵌套树匹配不到,已修为嵌套+Decode锚定容器解vlan同名歧义**
