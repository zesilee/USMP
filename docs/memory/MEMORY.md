...
- [EviewUI/openInula迁移调研](eviewui-inula-migration.md) — **已定案且缺料全闭环(2026-08-17,含全量24组件矩阵)**：全家桶拍板、alias实测=require("react")构建期别名即可、inula-X三段式同构zustand、EviewUI peer是horizon非react、inula-intl两头通吃t()零改动、inula-router是v5 API、**半受控是全库地基风险**(真受控仅Radio/Switch)、Menu无对应左树要自研、data-test全库不透传、令牌注入已通(CSS变量覆盖--brand-*零编译,勿用过滤脚本采非类型包)、适配层重估1500~2500行；做EviewUI/换库/运行时工作前必读，全文 docs/research/{eviewui-inula-migration,eviewui-component-matrix}.md
