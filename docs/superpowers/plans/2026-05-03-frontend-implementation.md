...
### Task 10: CRD API 接口与 useDeviceConfig Composable

**Files:**
- Create: `frontend/src/api/crd.ts`
- Create: `frontend/src/composables/useDeviceConfig.ts`
- Test: `frontend/test/composables/useDeviceConfig.test.ts`

- [ ] **Step 1: 编写 API 类型定义和接口**
...
export async function getSchema(module: string): Promise<Schema> {
  const res = await axios.get(`/api/crd/schema/${module}`)
  return res.data
}

export async function createConfig(cr: Partial<DeviceConfigCR>): Promise<DeviceConfigCR> {
  const res = await axios.post('/api/crd/configs', cr)
  return res.data
}

export async function updateConfig(name: string, cr: Partial<DeviceConfigCR>): Promise<DeviceConfigCR> {
  const res = await axios.put(`/api/crd/configs/${name}`, cr)
  return res.data
}

export async function deleteConfig(name: string): Promise<void> {
  await axios.delete(`/api/crd/configs/${name}`)
}

// SSE Watch API
export function watchConfigs(device: string, module: string) {
  const eventSource = new EventSource(`/api/crd/configs/watch?device=${device}&module=${module}`)
  return eventSource
}

export async function getNativeModels(): Promise<{ name: string, title: string, vendor: string }[]> {
  const res = await axios.get('/api/crd/models?type=native')
  return res.data.models || []
}
```

- [ ] **Step 2: 实现 useDeviceConfig Composable**

```typescript
// frontend/src/composables/useDeviceConfig.ts
import { ref, computed, onMounted, onUnmounted, watch } from 'vue'
import type { DeviceConfigCR, ConfigPhase, Schema } from '../api/crd'
import { getSchema, createConfig, updateConfig, deleteConfig, watchConfigs } from '../api/crd'

export function useDeviceConfig(deviceIp: string, module: string) {
  ...
  const crName = computed(() => `${deviceIp}-${module}`)

  function connectWatch() {
    eventSource = watchConfigs(deviceIp, module)
  ...
  async function save(desiredConfig: Record<string, any>) {
    if (!schema.value) throw new Error('Schema not loaded')

    const crData: Partial<DeviceConfigCR> = {
      metadata: {
        name: crName.value,
        labels: { device: deviceIp, module }
      },
      spec: {
        schema: schema.value,
    ...
    if (configCR.value) {
      await updateConfig(crName.value, crData)
    } else {
      await createConfig(crData)
    }
  }

  async function remove() {
    if (configCR.value) {
      await deleteConfig(crName.value)
      configCR.value = null
    }
  }
...
describe('useDeviceConfig', () => {
  it('should expose correct API', () => {
    const result = useDeviceConfig('192.168.1.1', 'openconfig-interfaces')
    expect(result.configCR).toBeDefined()
    expect(result.schema).toBeDefined()
    expect(result.isLoading).toBeDefined()
    expect(result.isSyncing).toBeDefined()
    expect(result.save).toBeInstanceOf(Function)
    expect(result.remove).toBeInstanceOf(Function)
    expect(result.refresh).toBeInstanceOf(Function)
  })

  it('should calculate correct crName', () => {
    const result = useDeviceConfig('192.168.1.1', 'openconfig-interfaces')
    // 通过私有实现验证
  })
})
...
Run: `cd frontend && npm run test -- test/composables/useDeviceConfig.test.ts`
Expected: PASS

- [ ] **Step 5: Commit**

```bash
git add frontend/src/api/crd.ts frontend/src/composables/useDeviceConfig.ts frontend/test/composables/useDeviceConfig.test.ts
git commit -m "feat(frontend): 实现 CRD API 接口和 useDeviceConfig Composable"
```

---

### Task 11: StatusBadge 状态徽章组件
...
describe('StatusBadge Component', () => {
  ...
  it('should render Updating status with spinner', () => {
...
### Task 15: ConfigPage 通用配置页面（使用 useDeviceConfig）
...
vi.mock('../../src/composables/useDeviceConfig', () => ({
  useDeviceConfig: () => ({
    configCR: { value: { status: { phase: 'Ready', actual: { data: [{ name: 'GE0/0/1', vlan: 100, enabled: true }] } } } },
    schema: { value: { title: '接口配置', fields: [], listFields: [] } },
    isLoading: { value: false },
...
- [ ] **Step 3: 实现 ConfigPage 页面（使用 useDeviceConfig）**
...
<template>
  <div class="config-page">
    <div class="page-header">
      <h2>{{ pageTitle }}</h2>
      <StatusBadge v-if="configCR" :phase="configCR.status.phase" />
      <el-tag v-if="error" type="danger">{{ error }}</el-tag>
    </div>
    ...
    <div class="toolbar">
      ...
      <el-button type="primary" :icon="Refresh" @click="handleRefresh" :loading="isSyncing">刷新</el-button>
    </div>

    <DynamicTable
      v-if="schema && configCR"
...
async function handleDelete(row: any, index: number) {
  try {
    ...
    list.splice(index, 1)
    await configHook.save({ data: list })
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEditing.value) {
      const index = list.findIndex(c => c.name === currentConfig.value.name)
      if (index >= 0) {
        list[index] = { ...currentConfig.value }
      }
    } else {
      list.push({ ...currentConfig.value })
    }
...
watch(selectedDevice, () => {
  if (selectedDevice.value) {
    configHook = useDeviceConfig(selectedDevice.value, module.value)
  }
})
