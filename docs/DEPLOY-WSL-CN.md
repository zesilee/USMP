...
在 **WSL2 Ubuntu + 华为内网（TLS 拦截代理）** 环境用 `docker compose` 全栈部署 uSMP
（simulator + backend + frontend）。已在 `DESKTOP-1CJ11AT` 实测跑通。
...
- 已 `git clone` 本仓库（私有仓库，用 `gh auth login` 或 PAT 鉴权）。
- 构建**不需要** `yang-models` 子模块（ygot 生成物已提交），**不需要** `.env`。
- 记下你的 WSL eth0 IP 和本地代理端口：
  ```bash
  ip addr show eth0 | grep 'inet '     # 例：172.24.160.109
  env | grep -i proxy                  # 例：http_proxy=http://127.0.0.1:3131
  ```
  下文用 `<WSL_IP>` 和 `<PORT>` 代指（如 `172.24.160.109` 和 `3131`）。

> ⚠️ 本地代理常监听 `127.0.0.1:<PORT>`，**容器里的 127.0.0.1 指向容器自己**，够不到宿主代理。
> 只要该代理监听在 `0.0.0.0`（多数是），容器就能用**宿主 IP** `<WSL_IP>:<PORT>` 访问它。
> 华为 `proxyjp.huawei.com:8080` 需账号密码，不适合免交互构建，用本地代理转发即可。

---

## 1. 处理 TLS 拦截（改三个 Dockerfile，仅本地）

内网代理会拦截 HTTPS 并换成华为自签根 CA，容器不信任 → apk `certificate not trusted`、
...
```dockerfile
ENV GOPROXY=http://mirrors.tools.huawei.com/goproxy/
ENV GOSUMDB=off
ENV GOINSECURE=*
```
...
```bash
ip -o -4 addr show      # 例：仅 eth0 172.24.160.0/20
docker network inspect bridge --format '{{range .IPAM.Config}}{{.Subnet}}{{end}}'
```

设 `/etc/docker/daemon.json`（`172.28.0.0/24` 避开常见华为内网段：172.24 宿主 / 172.30 企业 /
10.96.0.0/12 k8s / 192.168.39·49·59·76 minikube；按你机器实际调整）：
```bash
...
```bash
cd <仓库根>
PX=http://<WSL_IP>:<PORT>                                   # 例 http://172.24.160.109:3131
NOPX=localhost,127.0.0.1,.huawei.com,mirrors.tools.huawei.com
...
- `NO_PROXY` 含 `mirrors.tools.huawei.com` → go 走内部镜像直连、不绕外部代理。
