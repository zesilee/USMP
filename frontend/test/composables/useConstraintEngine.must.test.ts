...
describe('useConstraintEngine · must 跨字段校验（数据驱动，无硬编码）', () => {
  ...
  it('满足约束 → 无违例；违反 → 返回带消息的 violation', () => {
    const form: Record<string, any> = { suppress: 2000, reuse: 750, interval: 20 }
    const mustViolations = () => computeMustViolations(mustFields, form)
    expect(mustViolations()).toEqual([])

    form.reuse = 3000 // reuse>suppress → 违反
    expect(mustViolations().map((v) => v.path)).toContain('/d/reuse')
    expect(mustViolations().find((v) => v.path === '/d/reuse')!.message).toBe('reuse 必须小于 suppress')
  })

  it('无 message 的 must → 生成含字段标签的通用提示', () => {
    const form: Record<string, any> = { suppress: 2000, reuse: 750, interval: 15 }
    const mustViolations = () => computeMustViolations(mustFields, form)
    const v = mustViolations().find((x) => x.path === '/d/interval')
    expect(v).toBeTruthy()
  ...
  it('隐藏字段(when=false)的 must 不触发（YANG 语义：节点不存在）', () => {
    ...
    const form: Record<string, any> = { mode: 'off', x: 5 } // x 隐藏 → must 跳过
    const mustViolations = () => computeMustViolations(f, form)
    expect(mustViolations()).toEqual([])
  })
  ...
  describe('叶子未赋值 → 其 must 不适用（节点不存在语义）', () => {
    ...
    it('enum 叶（真实 FieldDef 形态）未选 → 无违例', () => {
      ...
      const mustViolations = () => computeMustViolations(realShape, form)
      expect(mustViolations()).toEqual([])
      form['statistic-mode'] = 'vlan-group-based' // 选了违反值 → 正常违例
      expect(mustViolations().map((v) => v.path)).toContain('/ifm/interfaces/interface/statistic-mode')
      form['statistic-mode'] = '' // 清空回未选 → 违例消失
      expect(mustViolations()).toEqual([])
    })

    it('未填（undefined/空串）→ 无违例，不再强迫用户选值', () => {
      const form: Record<string, any> = {}
      const mustViolations = () => computeMustViolations(smFields, form)
      expect(mustViolations()).toEqual([])
      form['statistic-mode'] = ''
      expect(mustViolations()).toEqual([])
    })
    ...
    it('填了值 → must 正常评估：满足无违例、违反有违例', () => {
      const form: Record<string, any> = { 'statistic-mode': 'interface-based' }
      const mustViolations = () => computeMustViolations(smFields, form)
      expect(mustViolations()).toEqual([])
      form['statistic-mode'] = 'vlan-based' // l2/statistic-enable 未开 → 违反
      expect(mustViolations().map((v) => v.path)).toContain('/ifm/interfaces/interface/statistic-mode')
    })

    it('引用他叶的 must：载体叶未填同样跳过（reuse 未填不评估 suppress>reuse）', () => {
      const form: Record<string, any> = { suppress: 2000 }
      const mustViolations = () => computeMustViolations(mustFields, form)
      expect(mustViolations().filter((v) => v.path === '/d/reuse')).toEqual([])
    })
    ...
    it('布尔 false 与数值 0 是有效值，must 照常评估', () => {
      ...
      const form: Record<string, any> = { n: 0 }
      const mustViolations = () => computeMustViolations(f, form)
      expect(mustViolations().map((v) => v.path)).toContain('/d/n')
    })
  })
  ...
  it('must 表达式解析失败 → 不阻断 + 记录告警（R08）', () => {
    ...
    const form: Record<string, any> = {}
    const mustViolations = () => computeMustViolations(f, form)
    const warnings = () => computeWarnings(f, form)
    expect(mustViolations()).toEqual([])
    expect(warnings().some((w) => w.includes('/d/bad'))).toBe(true)
  })
})
