...
2. 后端YANG模型、ygot结构体、DeviceActor/YANGActor完成后自动生成对应前端
3. 需要展示设备配置、编辑配置、下发配置时自动触发
4. 与后端所有技能联动：Actor、NETCONF、缓存、TDD

## 二、核心原则
1. **模型驱动UI**：YANG = 前端结构，不手写任何配置页面
2. **实时性**：每次打开表单自动从YANG Actor拉取最新配置
3. **无状态**：前端不保存任何配置，所有数据来自后端Actor
4. **类型自动映射**：
   - boolean → 开关
   - enum → 下拉选择框
   - string → 输入框
   ...
   - list → 表格 + 新增行
   - container → 分组面板
5. **一键下发**：提交后直接发给YANG Actor → NETCONF 下发设备
