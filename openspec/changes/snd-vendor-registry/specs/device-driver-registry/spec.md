# device-driver-registry delta — snd-vendor-registry

## ADDED Requirements

### Requirement: DR-01 编译期驱动描述符注册表

系统 SHALL 提供进程内驱动描述符注册表（`pkg/yang-runtime/driver`）：描述符携带 `Vendor`、`ModulePrefix`（路径匹配前缀集）、`Controller`（路由目标控制器名）与编解码闭包；SHALL 经 `init()` 编译期注册（无运行时插件加载），`Register`/`Lookup` 并发访问 SHALL 无数据竞态（R09）。`Lookup(vendor, path)` 未命中 SHALL 返回 `ok=false` 供调用方降级（R08），SHALL NOT panic。
