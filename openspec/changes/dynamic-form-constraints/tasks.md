...
- [ ] 3.1 [红/绿] `schema/entry.go` 识别 `IsChoice()/IsCase()` 建 `ChoiceNode/CaseNode`，成员子字段 **path 扁平化**（剥 choice/case 段、留 container 段）；`defaultChoice/defaultCase` 实现；B1 用 IFM `bandwidth-type`（双单叶 case）+ 嵌套 `damping→damp→level→manual`（container 段保留）
- [ ] 3.2 [红/绿] `field_gen` `nodeToNestedField` 加 `ChoiceNode` 分支→`FieldDef{type:"choice", cases:[{name,label,fields}]}`；`collectFields` 递归 choice cases；`FieldDef.Cases` + `CaseDef`；B2 集成：`bandwidth` 下发→回读收敛（写路径未破）
- [ ] 3.3 [红/绿] `FieldRenderer` choice→`el-tabs`(含多字段 case)/`el-radio-group`(全单叶)，切 case 清空非激活分支、payload 仅激活 case 扁平 path；F2 + F3 真浏览器（el-tabs teleport）
