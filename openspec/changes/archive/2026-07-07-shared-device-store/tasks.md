...
- [x] 6.2 主 spec 已按 SPEC_CONVENTIONS 标准迁移：config-api(#111)/yang-controller-runtime(#112) 迁移时折叠了本 change 的 delta 行为，新建 `openspec/specs/device-store/spec.md` 主 spec；change 手动 git mv 归档（自动 archive 因 delta 已折叠会重复，故手动）。全部 `openspec validate --type spec` 通过。
