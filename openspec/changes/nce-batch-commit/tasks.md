...
- [ ] 4.1 F2 红灯：攒批工具栏（FE-23）——四按钮渲染/禁用态、徽标计数、提示条出现与关闭、切设备隔离
- [ ] 4.2 实现工具栏组件接入 `ModuleConsolePage` 页头，绿灯；i18n `console.batch` 域双语补齐（zh/en 行号对齐）
- [ ] 4.3 F2 红灯：变更内容弹窗——树形三列、增/改/删着色与图例计数、空态
- [ ] 4.4 实现变更内容弹窗（纯前端渲染），绿灯
- [ ] 4.5 F2 红灯：试运行弹窗——双 Tab、正向/回滚双栏 XML、降级说明、基线标注、失败如实报错
- [ ] 4.6 实现试运行弹窗（调 preview 接口），绿灯
- [ ] 4.7 F3 真浏览器：工具栏弹窗开合、变更内容树展开、试运行 Tab 切换
- [ ] 4.8 覆盖率棘轮核对，commit + PR-4
