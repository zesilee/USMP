# yang-codegen-pipeline — delta (retire-ygot-runtime)
...
**Reason**: 自研生成器（CG-01 修订版）构造性满足两类后处理目标——枚举标识符合法化内建于生成器命名逻辑；确定性由生成器对无序集合的稳定排序内建保证，且 ygot 内嵌 gzip `ySchema` blob 随 Schema IR（YN-03）退役，不再存在需要规范化的 goyang Entry 序列化产物。`backend/tools/genfix` 随之退役。

**Migration**: 可复现性契约由 CG-01「同一仓库状态下重复执行输出字节一致」+ CG-03 regen-and-diff 门禁继续承载；枚举标识符合法化契约并入 CG-01 新增 Scenario「非法字符枚举标识符内建合法化」。
