...
- [ ] 8.6 行删除入变更集（FE-16）+ 归属徽标（FE-18）— **F2**

## 9. 布局与导航（PR-9）

- [ ] 9.1 左树：模块级展开、搜索过滤、rpc 入树、双语名（FE-13）— **F2**
- [ ] 9.2 Header / 侧边栏 / 面包屑 / 语言切换入口 — **F2**
- [ ] 9.3 react-router 路由表与重定向（对齐现有路由契约）— **F1 + F2**

## 10. 其余页面（PR-10、PR-11）

- [ ] 10.1 设备页（列表、表单、离线态）— **F2**
- [ ] 10.2 Dashboard（图表库届时再引入——echarts 已随清场移除死依赖）；**必须**在 `src/composables/useFleetOverview.ts` 同路径重建 hook 并恢复 `deriveOverview` 导出（沿用套件 `test/composables/useFleetOverview.test.ts` 复活，vitest include 同步加回）— **F1 + F2**
