...
- [ ] 2.2 [绿] `utils/moduleConsole.ts` 三纯函数 + `Field` 类型扩宽（supportFilter/operationExclude/presence）
- [ ] 2.3 [红] F2 `test/views/ModuleConsolePage.test.ts`：根子节点→Tab 派生（list/group/散叶聚合）、
      面包屑随激活 Tab、schema 加载失败降级
- [ ] 2.4 [绿] `views/ModuleConsolePage.vue` + 路由 `/module/:module`

## P3 · 列表 Tab 全功能（PR-2 主体，F2）
- [ ] 3.1 [红] F2 `ModuleListTab` 用 5 条 IFM 形 fixture（3 main/200GE/up + 2 sub/Vlanif/down）：
      模型列渲染、enum Tag、up/down 状态点、sub 行显示 parent-name / main 行 `-`
- [ ] 3.2 [红] 高级搜索：默认折叠、supportFilter 字段成控件、class=sub-interface 过滤 2 行、重置还原
- [ ] 3.3 [红] 分页：>pageSize 分页生效、切页、总数
- [ ] 3.4 [红] 操作门禁：编辑抽屉中 operationExclude∋update 字段禁用（新增态可编）；
      list 级 exclude 隐藏对应按钮
- [ ] 3.5 [绿] `components/config/ModuleListTab.vue`（drawer/对账流复用既有 composable）
- [ ] 3.6 review + commit

## P4 · 表单 Tab + presence（PR-2 收尾或 PR-3，F2）
- [ ] 4.1 [红] presence：`ipv4-conflict-enable` 形 group→开关、关=键不存在、
      `ipv4-ignore-primary-sub=true` 时开关禁用并强制关、求值失败降级可用
- [ ] 4.2 [红] 全局属性表单：statistic-interval range 10..600 + must mod10 违例行内报错并拦截提交
- [ ] 4.3 [绿] `FieldRenderer` presence 分支 + `components/config/ModuleFormTab.vue`（GET/POST 路径派生、后端不支持路径错误透出）
- [ ] 4.4 review + commit

## P5 · 导航/路由迁移 + 种子 + 冒烟（PR-3）
- [ ] 5.1 [红/绿] Sidebar 业务菜单模型驱动（menu store businessModules；失败回退硬编码，F2）
