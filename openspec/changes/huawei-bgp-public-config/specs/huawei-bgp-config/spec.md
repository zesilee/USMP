...
本期范围 SHALL 覆盖 `/bgp:bgp/global` + `base-process` 子树的**全部可配（rw）字段，无遗漏**（`global`：yang-enable、memory-overload-exception-discard-route、paf-controls/paf-control；`base-process` 直属全部标量 leaf：enable、as、keep-all-routes、check-first-as、router-id-auto-select、shutdown、local-ifnet-mtu、private-4byte-as、local-cross-no-med、as-path-limit、dynamic-session-limit、peer-up-route-lowest-priority、delay-time；子容器 confederation、graceful-restart、reference-period、timer、default-parameter 的全部可配字段）。系统 SHALL NOT 做字段挑选式覆盖（商用交付：覆盖部分字段即遗漏）。`base-process` 下的 config-false 只读态（vpn-brief-infos、graceful-restart-status、error-discard-info、remote-prefix-sid-states）SHALL NOT 作为配置下发目标，仅随生成物存在、可只读呈现。
