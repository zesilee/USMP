...
<!-- 无。BGP 作为「数据」流经既有通用引擎，不改变其 spec 级要求：
     - device-driver-registry：加一条描述符是注册数据，DR-01 描述符机制不变
     - yang-xml-codec：encode/decode/键式 delete 由 xmlcodec 按 ygot 数据驱动，XC-01~04 不变
     - translation-engine：token/route dispatch 由描述符谓词驱动，TE-05/06 不变
     - yang-codegen-pipeline：make gen-yang manifest 化后加模块名是数据，CG-01~03 不变
     这正是 P5「加模块=注册数据、零架构改动」的验证。若 augment 剪枝 spike（design.md）
     证伪该假设、需要把 network-instance 纳入生成，则回头补 yang-codegen-pipeline delta。 -->
...
- **风险**：唯一能推翻当前 gen.conf 方案的风险是 ygot 对 `augment /ni:...`（目标模块未纳入生成）的处理行为——详见 design.md，作为 tasks.md 第一红灯 spike。
