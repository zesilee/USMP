...
- **新增公网 BGP 配置面**：接入 `/bgp:bgp`（`huawei-bgp` 模块顶层独立根容器）下**全部 config-true（rw）标量 leaf，无遗漏，共 29 个**（以 SchemaTree config 继承为权威）——`global`（yang-enable、memory-overload-exception-discard-route）、`base-process` 直属 13 标量、config-true 子容器 `confederation`/`graceful-restart`/`reference-period`/`timer` 的标量。SchemaTree 入口 `HuaweiBgp_Bgp`，与 `HuaweiVlan_Vlan_Vlans` / `HuaweiIfm_Ifm_Interfaces` 同构（但为容器根，非 list 根——见 yang-xml-codec XC-05）。**不做字段挑选**，完备性由 schema 驱动用例保证。
  - **schema 核实纠正了两处早前臆断**：`default-parameter` 实为 **config false**（只读态，非早前列的 rw 子容器）；`paf-controls` 是 `global` 之**同级 list**（非其下标量）。二者本期均不接。
...
- `yang-xml-codec`: **ADD XC-05**——通用引擎新增 plain-container 根（根无 YANG list，如 `/bgp:bgp` 仅标量+子容器）的 encode/decode，与 list 根（vlan/ifm）共用同一字段级机制、按有无根 list map 字段自动择模式。**触发原因**：BGP 描述符往返真值测试（quality gate）实测 `Encode` 报 `container HuaweiBgp_Bgp has no list map field`——既有引擎是 list 中心设计（VLAN/IFM 均 list 根），从未走过容器根路径。这是「通用引擎其实不够通用」的第二处缺口（第一处=genfix schema 确定性），均由 BGP（容器根、多-augment 闭包）暴露、且被测试而非真机拦截。list 根行为不变（XC-01/02 golden 与往返恒等保持）。
