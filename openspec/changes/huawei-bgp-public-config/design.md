...
- **需评估的新权衡**：本 change 会引入 ~73k 行生成物（含尚未功能集成的 network-instance/acl/bfd/routing 等惰性类型）。生成物排除于 pr-size 门禁（R4），且 R04 regen-and-diff 保证合法性；但"生成大量未功能集成模型的类型"是否可接受，须用户确认（见 Open Questions）。

## Open Questions

- **[阻塞 D2/D4，spike 消解]** ygot 对未生成 augment 目标的确切行为？`HuaweiBgp_Bgp` 是否为实际入口 key 名？→ tasks 第一红灯项。
