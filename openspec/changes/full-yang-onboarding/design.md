...
- [desired 存储 TTL 1min，周期控制器对「TTL 过期后的带外漂移」无法修复] → 存量平台语义（risklog 已记），全量接入放大其影响面但不在本 change 改动存储语义；根治属 desired 持久化议题（K8s CRD 载体）另立 change
- [-ignore_unsupported 全局生效，未来 snd 升级可能静默丢不支持节点] → 评审确认接受：该开关只跳过解析期语句；deviation 文件仍是显式豁免账本，R04 regen-diff 会暴露生成物结构变化供复审
