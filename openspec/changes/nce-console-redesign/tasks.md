...
- [ ] 3.1 F2 红灯：FieldRenderer boolean→打开/关闭 radio（true/false 值语义、可选不选不入 payload、禁用态）
- [ ] 3.2 F2 红灯：约束合成 placeholder（range/length 文案、dynamicDefault 优先级不被覆盖、无约束无占位）
- [ ] 3.3 F2 红灯:key 叶钥匙图标+编辑态只读（创建态可填）；字段级清除（置空→不入 diff/payload、必填清除触发校验、tooltip 文案）
- [ ] 3.4 实现 FieldRenderer/表单层上述规范，绿灯；i18n 新文案双语补齐（UI-02 无残留硬编码）
- [ ] 3.5 三列栅格布局（窄视口降列、choice/leaf-list/嵌套表格整行、when 隐藏不占位）+ F2 断言
- [ ] 3.6 ModuleFormTab 嵌套 group→二级 Tab（FE-02 新形态），既有折叠断言改写
- [ ] 3.7 F3 真浏览器：radio/清除/占位在真实控件上的交互核验；`make e2e-local` 全绿
- [ ] 3.8 覆盖率棘轮核对，commit + PR-3

## 4. 左树搜索与展开收起（PR-4）

- [ ] 4.1 F2 红灯：左树搜索框（命中展开祖先链、清空恢复、不可用叶保禁用、无命中空态）与展开/收起全部按钮
- [ ] 4.2 实现 LeftTreeMenu/Sidebar 搜索与展开收起（复用 1.5 过滤纯函数；LT 生成物与接口零变更）
- [ ] 4.3 E2E 左树既有断言回归 + 新增搜索冒烟；`make e2e-local` 全绿
- [ ] 4.4 覆盖率棘轮核对，commit + PR-4
