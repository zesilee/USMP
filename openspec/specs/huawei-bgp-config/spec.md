# huawei-bgp-config — 华为公网 BGP 配置管理（Stack B）

## Purpose

华为公网 BGP 进程配置（`/bgp:bgp` 容器根，config-true 标量层，29 字段无遗漏）的模型驱动读写闭环——沿用 VLAN/IFM 驱动注册表 playbook：gen.conf 加 `huawei-bgp` → 描述符注册 → 通用 xmlcodec 编解码（含容器根 XC-05）→ BGP reconciler + 控制器注册 → B2 端到端集成。是 BGP 分期接入（peers/AF → per-VPN → feature）的地基，功能性配置依赖为零、可独立交付。由 change `huawei-bgp-public-config`（PR #146，2026-07-13）引入。
