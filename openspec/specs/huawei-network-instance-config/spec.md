# huawei-network-instance-config — 华为 network-instance（L3VPN 实例）配置管理（Stack B）

## Purpose

华为 network-instance（`/ni:network-instance` 容器根 + 嵌套 list，原生 config-true 面：`global` 3 标量 + `instances/instance` 的 name/description，共 5 个原生 config-true 标量无遗漏）的模型驱动读写闭环——沿用驱动注册表 + BGP 容器根 XC-05 playbook：gen.conf 加 `huawei-network-instance` → 单条描述符（namespace `urn:huawei:yang:huawei-network-instance`，谓词 `HasPrefix("/ni:network-instance")`）→ 通用 xmlcodec 容器根编解码（嵌套 list 零缺口，无需扩展 XC-05）→ networkinstance reconciler 容器根收敛 + 控制器注册 → B2 端到端集成。因 augment 合并，`/ni:network-instance` 是单一 ygot 根，是 BGP 二期一切 peering（peers/afs/peer-groups 均 augment 于此根下）的**唯一硬前置**，未来 2a peers 扩展同一描述符驱动面。config-false 回读态与设备侧 instance 删除（DELETE 命令通道）为推迟债。由 change `huawei-network-instance-config`（PR #148/#149，2026-07-13）引入。
