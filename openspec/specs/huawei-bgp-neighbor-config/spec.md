# huawei-bgp-neighbor-config — 华为公网 BGP 基础邻居配置管理（Stack B）

## Purpose

华为公网 BGP peer（`/ni:network-instance/instances/instance[_public_]/bgp/base-process/peers/peer`，key=`address`）的模型驱动读改下发闭环：41 config-true 标量（含 remote-as/description/connect-mode/tcp-mss/password/tracking 等 + timer/graceful-restart/bfd-parameter 子容器）+ `afs/af` 的 af-type 枚举 key。**核心：复用 network-instance 描述符与 reconciler**（`/ni:network-instance` 是单一 ygot 根，peer 是其 huawei-bgp augment 子树），零新描述符/控制器；经 XC-06 per-node namespace 使 `<bgp>`↓ 带 huawei-bgp namespace、XC-07 支持 empty 类型（bfd-parameter/compatible）。策略属性（route-policy/filter/ACL/tunnel）、peer-groups、per-VPN peers 属 2b/后续。由 change `huawei-bgp-neighbor-config`（PR #153，2026-07-13）引入。
