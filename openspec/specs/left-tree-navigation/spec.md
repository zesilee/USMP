# left-tree-navigation — SND 左树导航

## Purpose

原生配置左侧导航以 SND 包 left-tree.json 为组织权威（14 业务域分组/3 层/65 特性模块/中英双语）：构建期 codegen 固化树与叶子模块映射（运行镜像零 snd 文件依赖），运行期标注每叶可用性（随模块渐进生成自动变真）与按设备能力支持度，前端全树渲染（未接入占位）并在接口异常时回退任务域分组导航。
