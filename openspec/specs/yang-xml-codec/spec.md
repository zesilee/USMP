...
通用 NETCONF XML 编解码引擎（`pkg/yang-runtime/xmlcodec`）：任意自研 Object
（yang-native-runtime YN-01）的 encode / decode / 键式 delete 编码，数据来源仅生成物
（`path:`+`module:` struct tag、Schema IR 节点、`KeyedObject` key 元数据）与驱动描述符
登记的编解码数据（模块 namespace 为显式数据）。新增 YANG 模块零 XML 代码，是 SND
声明式化（P5）的编解码承载层。由 change `snd-xml-codec` 引入（PR #136/#137/#138），
retire-ygot-runtime 完成 schema 源 IR 化与接口族置换。
## Requirements
