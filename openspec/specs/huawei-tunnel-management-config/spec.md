# huawei-tunnel-management-config — 华为 tunnel-management 配置管理（Stack B）

## Purpose

华为 tunnel-management（`/tnlm:tunnel-management` 容器根，标量边界：`tunnel-policys/tunnel-policy` 的 name+description 嵌套 list + `tunnel-down-switch/enable`）的模型驱动读写闭环——沿用 VLAN/IFM/BGP 驱动注册表 playbook：一期全闭包已生成结构体（零 codegen）→ 描述符注册（显式 namespace + SchemaTree 入口）→ 通用 xmlcodec 编解码（容器根 XC-05 + per-node ns XC-06）→ tunnelmgmt 容器根 reconciler → B2 端到端集成。是 BGP 二期-2b DAG 的叶子前置：使 tunnel-policy 实例按 name 存在，解除 BGP AF `tunnel-policy` leafref（require-instance=true）的越序阻塞，同时是 routing-policy（2b 波次③）前置。深层 `ipv4-set`/`ipv6-set`（choice/presence/ordered-by/ifm leafref）显式推迟 follow-up。由 change `huawei-tunnel-management-config`（2026-07-14）引入。
