# huawei-xpl-config — 华为 xpl route-filter 配置管理（Stack B）

## Purpose

华为 xpl `route-filters/route-filter`（`/xpl:xpl` 容器根，name + content 标量）的模型驱动读写闭环——沿用 VLAN/IFM/BGP/tunnel-management 驱动注册表 playbook：一期全闭包已生成结构体（零 codegen）→ 描述符注册（显式 namespace + SchemaTree 入口 HuaweiXpl_Xpl）→ 通用 xmlcodec 编解码（容器根 XC-05 + per-node ns XC-06）→ xpl 容器根 reconciler → B2 端到端集成。是 BGP 二期-2b DAG 的叶子前置：使 route-filter 实例按 name 存在，解除 BGP AF `route-filter` leafref（require-instance=true）的越序阻塞。xpl 其他策略 list（as-path/community/prefix/rd/large-community 等）非本 leafref 目标，显式推迟 follow-up。由 change `huawei-xpl-config`（2026-07-14）引入。
