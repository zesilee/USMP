...
北向「网络自动化服务」以 CRD 发布业务配置契约。本能力提供构建期工具 `tools/crd2yang`（`tools/crdgen` 的镜像）：把北向拥有的 CRD 反向派生为业务网络配置 YANG 模型，接入既有管线（ygot→schema 树→R05 渲染）。authority 边界：USMP 自有意图模型仍 YANG-first（BIC-01 不变），同一模型禁止双向都跑。
