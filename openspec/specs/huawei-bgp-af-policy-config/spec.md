# huawei-bgp-af-policy-config — 华为 BGP AF 策略属性配置管理（Stack B）

## Purpose

华为 BGP AF `import-filter-policy` 策略属性（`/ni:.../instance[_public_]/bgp/base-process/afs/af[type]/ipv4-unicast/import-filter-policy` 下 `acl-name-or-num`→acl group、`filter-name`/`filter-parameter`→xpl route-filter 的可选 leafref）的模型驱动配置管理——**复用既有 network-instance 描述符与 reconciler**（2a 已覆盖整棵 ni 子树含 huawei-bgp augment）+ XC-06 per-node namespace（`<bgp>` 带 huawei-bgp namespace），**零新描述符/reconciler/codegen**。是 BGP 二期-2b 的收官：波次①②③④（tunnel-management/xpl/routing-policy/acl）+ XC-08 使四类目标模型全可配、越序禁令解除后，兑现 2a 排除的 AF 策略属性，证明 2b 全链路（AF 属性→已集成目标 leafref→经 ni 链路端到端下发）。深层 route-policy/tunnel-policy、ipv4-prefix-filter（rtp prefix-filters 未集成）、其他地址族/peer 级策略显式推迟 follow-up。leafref 编排依赖（目标先配）登记。由 change `huawei-bgp-af-policy-attrs`（2026-07-14）引入。
