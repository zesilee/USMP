...
生成管线 SHALL 使用 Go 实现的后处理器（`backend/tools/genfix`）修复 ygot 生成的含非法字符 `|` 的枚举标识符（如 `PortType_50|100GE` → `PortType_50_OR_100GE`），SHALL 在 Linux 与 macOS 上行为一致（不依赖平台 sed 方言）。修复 SHALL 幂等：对已修复或无匹配的输入执行为 no-op；SHALL NOT 改动枚举标识符之外的内容（含 YANG 原值字符串映射）。
