...
`src/ui` 是前端业务代码与具体 UI 组件库之间的唯一隔离层：业务代码只从适配层导入控件/图标/反馈 API/主题令牌，组件库（当前 Ant Design 6）成为单点可替换依赖——为将来切换公司内部组件库（EviewUI）保留能力。适配层是薄转发（控件重导出 + API 差异抹平 + 图标与令牌收口），不承载业务语义。
