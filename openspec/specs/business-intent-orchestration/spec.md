# business-intent-orchestration Specification

## Purpose

业务意图编排：CRD watch（C4）→ 意图 Reconciler（C3）→ 展开为每设备原生片段 → 跨设备 2PC 下发 → 声明式稳态；生命周期（finalizer 删除/收缩差集）与软归属认领。CRD 仅当持久化+watch 载体，Stack B 对账架构不变（R01）。
