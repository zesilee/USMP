...
next: P5 后续步骤（每步独立 change）：② XML builder/parser 声明式化（最大块，golden test 先行）→ ③ ygot 生成管线参数化 → ④ 首个非 Huawei 驱动落地验证注册表
---
...
### [x] P1' — NCE 保真微调（洞察 G + F）｜已完成（PR #132 合入 + sync + 归档，2026-07-09）
- [x] 类型→控件主映射已在 `FieldRenderer.vue`（string/number/boolean/enum/group/presence/leaf-list/choice/list）
- [x] enum→`el-segmented`：**必填且 ≤3 选项**才走分段（segmented 无 clearable，可选枚举需保留「清空=键不入 payload」）；FE-01 MODIFIED 已 sync 主 spec
- [x] 高密度表格：theme.scss 密度令牌（--table-cell-py/px/--table-font-size）全局覆盖，全站 el-table 自动继承；tokens 契约测试锁定
- [x] FieldRenderer 内联 hex 令牌化（真正的 redesign 对齐债）
- ~~leafref → `el-transfer`~~ **剪出**：后端 `mapLeafType` 把 leafref 落为 string（entry.go:437），契约无该类型；需后端新 LeafType+前端动态拉引用候选集=独立 feature
- ~~主色换 `#307FE2`~~ **不做**：估读色不推翻已批准 redesign 的 `#0C5EA6` 体系（决策在归档 change 的 proposal）
...
**剩余步骤（每步独立 change）**：
- [ ] P5-2 XML builder/parser 声明式化（netconf.go 45KB 手写 build/Parse/map 三件套/模型——最大块，**golden test 先行**再重构；descriptor 扩展路径/模板描述）
- [ ] P5-3 ygot 生成管线参数化（Makefile 化、多厂商 -path、消灭手工 sed 四连击；注意 huawei.go:9 -path 注释漂移）
- [ ] P5-4 首个非 Huawei 驱动（真实验证注册表端到端；需该厂商 YANG 模型集 + netconfsim 方言）；mergeConfig/config_delete 的 per-model type-switch 收敛也在此步或 P5-2 一并
...
#### explore 存量审计结论（2026-07-09，代码级核对 @main，propose 输入）
- **vendor 概念在生产链路里不存在**：`DeviceConnectionInfo`（client/client.go:91）只有 IP/Port/凭据/Protocol，**无 Vendor 字段**；协议按端口判（pool.go:125，830→NETCONF/9339→gNMI）；唯一"厂商识别"是只读 schema 路径的 namespace 猜测（field_gen.go:186 vendorForNamespace），不流入下发链路
- **translator 是半成品注册表**：`factory.go:21` `once.Do` 硬编码只注册 Huawei；调用点仅 crdsource 两处且写死 `VendorHuawei` 常量；输入契约绑死 `bizv1.*` CRD Spec（Stack A 遗留），但 crdsource 本身是 Stack B 生产路径（main.go:86 wire）
- **要消灭的硬编码分支清单**（加新 (vendor,model) 全要动）：A. per-model type-switch 6+ 处（netconf.go:431/1056 marshalChange、config_handler.go:283/251、config_delete.go:123、3 个 reconciler 断言）；B. path-string dispatch 6 处（config_codec.go:28/71、config_handler.go:313、config_delete.go:24、**manager.go:325 路径→控制器路由**）；C. **手写 per-model XML builder/parser 是最大块**（netconf.go 45KB：build/Parse/map 每模型三件套 + 巨型字段 switch）
- **plugin 钩子生产零消费**（AddPlugin 无生产调用）——厂商特定 Validation/Mutation 可直接落现成接口，无需新框架
- **ygot 生成管线双轨手工**：生产用 internal/generated/huawei 的 go:generate+sed 四连击（无 Makefile target、-path 注释已漂移）；另有 internal/yang/generate.go 疑似遗留旧管线
- **最小可行第一步（PR ≤1000 行）建议**：①`DeviceConnectionInfo`+device.Store 加 Vendor/Model 贯穿（默认 Huawei 行为不变）②translator 改 init() 编译期自注册、调用点读 Vendor ③新建 `(vendor,path-prefix)→descriptor` 注册表，先收敛 manager.go 路由与 config_codec 编解码表。**不碰** XML builder（需 golden test 先行）、ygot 管线参数化、plugin 落地——留后续 PR
