...
func (h *BusinessHandler) client(c *beecontext.Context) client.Client {
	cl := h.clientFn()
	if cl == nil {
...
func (h *BusinessHandler) List(c *beecontext.Context) {
	cl := h.client(c)
	if cl == nil {
		return
	}
...
func (h *BusinessHandler) Get(c *beecontext.Context) {
	cl := h.client(c)
	if cl == nil {
		return
	}
	...
	err := cl.Get(c.Request.Context(), types.NamespacedName{Namespace: h.namespace, Name: c.Input.Param(":name")}, u)
	if apierrors.IsNotFound(err) {
		Error(c, 404, "意图实例不存在: "+c.Input.Param(":name"))
		return
	}
	if err != nil {
		Error(c, 502, "读取意图实例失败: "+err.Error())
...
func (h *BusinessHandler) Apply(c *beecontext.Context) {
	cl := h.client(c)
	if cl == nil {
		return
	}
	...
	if req.Name == "" || req.Spec == nil {
		Error(c, 400, "请求体不合法: name 与 spec 均为必填")
		return
	}
...
func (h *BusinessHandler) Delete(c *beecontext.Context) {
	cl := h.client(c)
	if cl == nil {
		return
	}
	...
	Success(c, map[string]interface{}{"name": c.Input.Param(":name")}, "deletion accepted (device cleanup via finalizer)")
}
