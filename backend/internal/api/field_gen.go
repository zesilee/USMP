...
func buildYangSchema(mod schema.Module) YangSchema {
	...
	ys.RPCs = rpcSchemasFor(mod.Name())
	return ys
}
...
// rpcSchemasFor returns the module's rpc presentation schemas (RPC-02). ModuleRPCs
// (generated by rpcgen, RPC-01) is keyed by root container name — the same key
// mod.Name() carries — so lookup is direct. Input leaves reuse FieldDef so the
// front-end renders them with the existing config pipeline; a leafref input keeps
// its target in LeafRef for a value dropdown (Phase③).
func rpcSchemasFor(module string) []RPCSchema {
	defs := yangschema.ModuleRPCs[module]
	if len(defs) == 0 {
		return nil
	}
	out := make([]RPCSchema, 0, len(defs))
	for _, d := range defs {
		r := RPCSchema{Name: d.Name, Label: d.Name, HighRisk: d.HighRisk}
		for _, in := range d.Input {
			r.Input = append(r.Input, rpcInputField(in))
		}
		out = append(out, r)
	}
	return out
...
// rpcInputField maps a generated rpc input leaf to a FieldDef.
func rpcInputField(in yangschema.RPCInputLeaf) FieldDef {
	typ := in.Type
	if typ == "leafref" {
		// A name reference is a string to the base renderer; LeafRef carries the
		// target so Phase③ can upgrade it to a dropdown.
		typ = "string"
	}
	return FieldDef{
		Path:     in.Name,
