...
func TestBuildHuaweiVlanVlansXML_SingleVlan(t *testing.T) {
	...
	assert.Contains(t, result, `<broadcast-discard>enable</broadcast-discard>`)
	assert.Contains(t, result, `<mac-learning>disable</mac-learning>`)
	assert.Contains(t, result, `<statistic-enable>disable</statistic-enable>`)
	assert.Contains(t, result, `</vlan>`)
