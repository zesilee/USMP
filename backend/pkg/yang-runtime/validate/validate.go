// Package validate 是基于 Schema IR 的服务端校验器（retire-ygot-runtime YN-04，
// 任务5.2）：pattern（锚定全匹配）/range/length/min-elements 四类叶约束，替代
// ygot 生成的 Validate()。
//
...
// Object validates a generated YANG object subtree against its schema node.
// v 为容器指针（如 *BusinessVlanService），n 为对应 schema 节点。
func Object(n schema.Node, v interface{}) error {
...
func walkStruct(n schema.Node, sv reflect.Value, path string) error {
	st := sv.Type()
	for i := 0; i < st.NumField(); i++ {
		...
		case reflect.Map:
			if ln, ok := child.(schema.ListNode); ok && fv.Len() == 0 && ln.MinElements() > 0 {
...
func checkLeaf(n schema.Node, v reflect.Value, path string) error {
	leaf, ok := n.(schema.LeafNode)
	if !ok {
...
func checkRange(leaf schema.LeafNode, val int64, path string) error {
...
func leafLengths(leaf schema.LeafNode) ([2]int, bool) {
...
func childNode(n schema.Node, name string) schema.Node {
	if n == nil {
		return nil
	}
